      ...
      <ul>
        {data.results.map((pokemon: any, index: number) => (
          <Link to={`/pokemon/${pokemon.name}`} key={pokemon.name}>
            <li className="border-2 border-black dark:border-white shadow-sm w-auto px-20 mb-1 bg-slate-700 text-white text-base hover:text-yellow-300 p-2">
              {pokemon.name}
            </li>
          </Link>
        ))}
